...
@st.cache_data(ttl=300, show_spinner="載入資料庫結構…")
def load_schema() -> dict:
    ...
    try:
        ...
        """)
        for tbl, tdesc, col, dtype, nullable in cur.fetchall():
...
if schema:
    table_names = [k for k in schema if not k.startswith("_")]
    with st.expander(f"📋 可查詢的資料表／檢視表（共 {len(table_names)} 個）"):
